...
get_PCA_contributors <- function(in_df, n_pc = 1, n_factors  = 10){
  ...
  tops <- lapply(pcs, function(x) get_top_contributor(pca_df, which_PC = x,
                                                      n_out = n_factors,
                                                      return_df = TRUE))
...
make_PCA_contributor_plot <- function(tops){
  ...
  P <- ggplot(tops, aes(x = id, y = value)) +
    ...
    facet_wrap(~PC, scales = "free") + ylab("PC loading") + xlab("")
...
get_top_contributor <- function(pca_df, which_PC = "PC1", n_out = 6, return_df = FALSE){
  ...
  if(! "PC" %in% names(pca_df)){
    stop("The df you supplied does not contain a column named 'PC'. Make sure
         get_PCA_contributors() instead which takes care of that.")
  }
      ...
      if(j <= ncol(x)){
        if(!any(coldataTypes[[i]] %in% c("factor","character"))){
          ...
          if(length(levels(groups)) > 1){
            ...
            warning(paste("Whatever is in", colnames(coldata)[i], "can neither be used for a Kruskal Test nor a correlation. Check the content; it should be either continuous or factors or characters."))}
        }
      }else{ # j > ncol
        warning(paste("PC", j, "is not part of the PCA results and will be ignored."))
      }
    }# j in pcs
  } # i in ncol(res)
  return(res)
...
plot_pca_deseq <-function(object, pca_results, pc_x = "PC1", pc_y = "PC2",
  ...
  df_to_plot <- data.frame(x = pca_results[, pc_x],
                           y = pca_results[, pc_y],
  ...
  # make the plot ------------------------------------------------------------
  out_plot <- plot_pca.df(df_to_plot, color_by=color_by, shape_by=shape_by,
...
plot_pca.df <- function(df_to_plot,
  ...
  if ( is.null(percentVar) ) {
    ...
    x_lab <- paste0(comps[1], ": ", round(percentVar[1] * 100),
                    "% variance")
    y_lab <- paste0(comps[2], ": ", round(percentVar[2] * 100),
                    "% variance")
  }
  ...
  if(is.numeric(df_to_plot[,shape_by])){df_to_plot[, shape_by] <- factor( df_to_plot[, shape_by])}
  
  plot_out <- ggplot(df_to_plot,
                     aes_string(x = comps[1],
                                y = comps[2],
...
fx.get_circle_dt <- function(in.df, fill_by = NULL, which_quant = 1){
  ...
  ABCutilities::check_columns(fill_by, in.df,
  ...
  out.dt <- pdt[get(comps[1]) >= x.low & get(comps[1]) <= x.high &
...
fx.get_palette_ABC <- function(palette_name) {
  switch(palette_name,
         ...
         tableau20 = c("#34B20D","#FFAE18", "#be2f00","#73FFC3" , "#0D14B2",
                       ...
                       c( "#FF10FC" , "#3E68FF", "#8B9440", "#7F85C3", "#FF85C3",
