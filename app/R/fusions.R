...
importStarfusion2 <- function (filename, genomeVersion, limit)
{
  ...
  report <- tryCatch({
    col_types_starfusion = readr::cols_only(`#FusionName` = col_skip(),
  ...
  for (i in 1:dim(report)[1]) {
    ...
    fusionToolSpecificData[["LargeAnchorSupport"]] = report[[i,
                                                             "LargeAnchorSupport"]]
    fusionToolSpecificData[["LeftBreakDinuc"]] = report[[i,
                                                         "LeftBreakDinuc"]]
    fusionToolSpecificData[["LeftBreakEntropy"]] = report[[i,
                                                           "LeftBreakEntropy"]]
    fusionToolSpecificData[["RightBreakDinuc"]] = report[[i,
                                                          "RightBreakDinuc"]]
    fusionToolSpecificData[["RightBreakEntropy"]] = report[[i,
                                                            "RightBreakEntropy"]]
    id <- as.character(i)
    ...
    geneA <- new(Class = "PartnerGene", name = nameA, ensemblId = ensemblIdA,
    ...
    geneB <- new(Class = "PartnerGene", name = nameB, ensemblId = ensemblIdB,
...
plotCircle2 <- function(fusionList, cytoband) {
  ...
  names(cytoband) <- c("Chromosome", "ChromStart", "ChromEnd",
                       "Band", "Stain")
  assign("RCircos.Env", RCircos::RCircos.Env, .GlobalEnv)

  cytoband <- RCircos.Sort.Genomic.Data(genomic.data = cytoband,
                                        is.ideo = TRUE)
  cyto.info <- cytoband
  chr.exclude <- NULL
  tracks.inside <- 3
  ...
  track.num <- 2
  RCircos::RCircos.Gene.Name.Plot(geneLabelData, name.col, track.num,
                         side)
