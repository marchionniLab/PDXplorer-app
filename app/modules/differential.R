...
diffInput <- function(id) {
  ...
  comp_a <- tags$div(selectizeInput(
    inputId = ns("comp_a"),
    label = "Group a",
    choices = NULL,
    selected = NULL,
...
diffOutput <- function(id) {
  ...
  tagList(
    ...
    tabsetPanel(
      ...
      tabPanel("Heatmap",
        shinydashboard::box(
          width = 12, title = "Gene expression heatmap",
          fluidRow(
            column(
              width = 3,
        ...
        shinydashboard::box(
          ...
          fluidRow(
            ...
            column(
              ...
              selectInput(ns("col"),
                "Point color (4):",
                choices = colors(), selected = c("goldenrod1", "black", "tomato2", "maroon4"),
                multiple = TRUE
              )
            ),
            # column(width=3,
          ...
          fluidRow(
            ...
            column(
              ...
              selectInput(ns("legendPosition"),
                "Legend position:",
                choices = c("top", "bottom", "left", "right"), selected = "bottom",
                multiple = FALSE
              )
            ),
            column(
...
diffMod <- function(input, output, session, dds = dds, metadata, fit = fit, fit_pdx = fit_pdx, fit_primary = fit_primary) {
  ...
  timer <- reactive(list(
    ...
    padj = input$padj,
    lfc = input$lfc,
    top_n_from_de_genes = input$top_n_from_de_genes
  )) %>% debounce(0)
  ...
  output$venn_plot <- renderCachedPlot({
    ...
    if (input$split_by_type == TRUE) {
      ...
      venn.plot <- VennDiagram::venn.diagram(ll,
        ...
        cex = 1, cat.fontface = 2, filename = NULL, margin = 0.1
      ) # , fill = c("dodgerblue", "goldenrod1", "red", "green"))
      ...
      p2 <- ggplot(merged_dfs, aes(x = logFC.x, y = logFC.y, color = color, label = Row.names)) +
  ...
  makeHeatPlot <- function() {
    ...
    if (input$split_by_type == FALSE) {
      ...
      if (input$keep_intersection == TRUE & input$keep_row_order == TRUE) {
        ph2 <- pheatmap::pheatmap(df_primary[row.order, ],
          cluster_rows = F,
          cluster_cols = input$cluster_col,
          cutree_cols = input$cutree_cols,
          main = "Primary",
        ...
        ph2 <- pheatmap::pheatmap(df_primary,
          cluster_rows = input$cluster_row,
          cluster_cols = input$cluster_col,
          cutree_cols = input$cutree_cols,
          main = "Primary",
  ...
  callModule(ggDownload, "ggvol", reactive({
    makeVolPlot()
  }))
  callModule(ggDownload, "ggheat", reactive({
    makeHeatPlot()
  }))
  callModule(ggDownload, "ggma", reactive({
    makeMaPlot()
  }))
}
