...
diffInput <- function(id) {
  ...
  select_contrast_by <- selectInput(
    inputId = ns("select_contrast_by"),
    label = "Select group",
    choices = NULL,
    selected = NULL
  )
        ...
        tabsetPanel(
          ...
          tabPanel(
            ...
            div(
              id = ns("scatter-outer"),
              shinydashboard::box(
                title = "Scatter plot", width = 12, id = ns("scatterPlotBox"),
          ...
          tabPanel(
            ...
            div(
              id = ns("md-outer"),
              shinydashboard::box(
                title = "Mean-difference plot", width = 12, id = ns("MDplotBox"),
          ...
          tabPanel(
            ...
            div(
              id = ns("volcano-outer"),
              shinydashboard::box(
                title = "Volcano plot", width = 12, id = ns("volcanoPlotBox"),
...
diffMod <- function(input, output, session, dds, metadata, deg, go, keggNames, keggLinks, ann) {
  ...
  output$upsetr_table <- DT::renderDataTable(
    ...
    server = TRUE,
    rownames = FALSE,
    options = list(scrollX = TRUE, search = list(regex = TRUE, caseInsensitive = TRUE), pageLength = 20, selection = list(mode = "single", target = "cell"))
  )
  ...
  output$volcanoPlot <- renderPlot({
    ...
    p <- ggplot(data = gene_list, aes(x = logFC, y = -log10(adj.P.Val), colour = threshold, key = row.names(gene_list))) +
      ...
      scale_colour_manual(name = "match filters", values = c("#808080", "#D75F1C")) +
      geom_hline(yintercept = -log10(as.numeric(input$pval_cutoff)), linetype = "dotted") +
      geom_vline(aes(xintercept = -as.numeric((input$logfc_cutoff))), linetype = "solid") +
      geom_vline(aes(xintercept = input$logfc_cutoff), linetype = "dashed") +
      geom_vline(aes(xintercept = 0), linetype = "solid", size = 0.2, show.legend = FALSE) +
      ggtitle(input$selected_contrast)
    p
  })
  ...
  output$MDplot <- renderPlot({
    ...
    p <- ggplot(data = gene_list, aes(x = AveExpr, y = logFC, colour = threshold, key = row.names(gene_list))) +
      ...
      geom_hline(aes(yintercept = -as.numeric((input$logfc_cutoff))), linetype = "solid") +
      geom_hline(aes(yintercept = input$logfc_cutoff), linetype = "dashed") +
      ggtitle(input$selected_contrast)
    p
  })
  ...
  output$scatterPlot <- renderPlot({
    ...
    p <- ggplot(data = df, aes(x = get(first_sel), y = get(second_sel), colour = threshold)) +
      ...
      ylab(paste0("log2(AveExpr) - ", second_sel)) +
      theme_bw(base_size = 14) +
      theme(legend.position = "bottom") +
      scale_colour_manual(name = "match filters", values = c("#808080", "#D75F1C")) +
      geom_hline(aes(yintercept = 0), linetype = "solid", size = 0.2) +
      geom_vline(aes(xintercept = 0), linetype = "solid", size = 0.2) +
      geom_abline(intercept = as.numeric(input$logfc_cutoff), linetype = "dashed") +
      geom_abline(intercept = -as.numeric(input$logfc_cutoff), linetype = "solid") +
  ...
  output$go_table <- DT::renderDataTable(
    {
      ...
      if (input$ontology == "all") {
        selected_ontology <- c("BP", "CC", "MF")
      } else {
        selected_ontology <- input$ontology
      }
