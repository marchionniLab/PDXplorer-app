...
#  pcaOut <- reactiveValues(dds = NULL, pca_mat = NULL, pcres = NULL, pccoords = NULL)
  mydds <- reactiveValues(dds = NULL, retain_samples = NULL, remove_samples = NULL, include_samples = NULL, redraw = TRUE, final_list_samples_to_include=NULL)
  ...
  pca_data <- reactive({
    ...
    withProgress(message = 'Running principal component analysis', {
      ...
      object = dds[,mydds$final_list_samples_to_include]
      rv <- rowVars(assay(object, "vst"))
      select <- order(rv, decreasing = TRUE)[seq_len(min(input$nr_genes, length(rv)))]
      pca <- prcomp(t(assay(object, "vst")[select, ]))
      percentVar <- pca$sdev^2/sum(pca$sdev^2)
      names(percentVar) = colnames(pca$x)
      intgroup = "TCL.subtype"
      d <- data.frame(pca$x,  colData(object), name = colnames(object))
      attr(d, "percentVar") <- percentVar
      return(d)
    })
  }) %>%
    bindCache(mydds$final_list_samples_to_include,input$nr_genes)
  ...
  output$pca <- renderPlot({
    ...
    percentVar <- c(attributes(d)$percentVar[input$pc_x], attributes(d)$percentVar[input$pc_y])

    # if color/shape aren't already present, add the respective gene values
    if(!is.null(n2n(input$color_by))){
      df_to_plot <- fx.add_gene_expression(n2n(input$color_by), df_to_plot, dds[,mydds$final_list_samples_to_include])
    }
    if(!is.null(n2n(input$shape_by))){
      df_to_plot <- fx.add_gene_expression(n2n(input$shape_by), df_to_plot, dds[,mydds$final_list_samples_to_include])
    }

    if (input$color_palette == "auto"){ set_colors = T } else { set_colors = F }

    p <- plot_pca.df(df_to_plot, color_by=n2n(input$color_by),
                     shape_by=n2n(input$size_by),
                     size_by=n2n(input$shape_by),
                     circle_by=n2n(input$encircle),
                     remove_rug = input$remove_rug,
                     percentVar = percentVar,
                     theme_size = 14, legend = "auto",
                     alpha = .8, set_colors = set_colors)
    ...
    if(input$color_by != "none"){
      if ( length(unique(colData(dds[,mydds$final_list_samples_to_include])[[input$color_by]])) > length(ggsci_db[[input$color_palette]]$default) ){
        color_set = colorRampPalette(ggsci_db[[input$color_palette]]$default)(  length(unique(colData(dds[,mydds$final_list_samples_to_include])[[input$color_by]]))  )
      } else {
        color_set = as.vector( ggsci_db[[input$color_palette]]$default[1: length(unique(colData(dds[,mydds$final_list_samples_to_include])[[input$color_by]]))])
      }
    }
