...
gseaInput <- function(id) {
  ...
  gene_set_db <- selectizeInput(ns("gene_set_db"),
    ...
    choices = gsub(".gmt", "", list.files("../data/msigdb/")),
    selected = NULL,
    multiple = FALSE
  )
  ...
  top_n <- numericInput(
    ns("top_n"),
    label = "Include top n most variable gene sets: ",
    value = 25, min = 1, max = 2000, step = 10
  )

  de_only <- checkboxInput(
    inputId = ns("de_only"),
    label = "Sort by statistical significance? Valid only when more than one subtype is selected.",
    value = TRUE
  )
  ...
  select_all <- actionLink(
    inputId = ns("select_all"),
    label = "Select all",
    style = "color:#18cc7b"
  )
  deselect_all <- actionLink(
...
gseaOutput <- function(id) {
  ...
  tagList(
    shinydashboard::box(
      width = 12, title = "Gene set variation analysis heatmap",
      fluidRow(
        column(
          width = 2,
      ...
      fluidRow(
        column(
          ...
          selectInput(ns("column_annotation"),
            label = "Annotate column by:", choices = NULL,
            selected = NULL, multiple = TRUE
          )
        ),
        column(
          width = 3,
          numericInput(ns("fontsize_row"),
            label = "Row fontsize :", value = 8,
            min = 1, step = 1
          )
        ),
        column(
          width = 3,
          selectInput(ns("cutree_cols"),
            label = "Cut cols into N groups:", choices = seq(1, 20), selected = 1,
            multiple = FALSE
          )
        ),
        column(
      ...
      helpText(
        paste(
          ...
          "Conceptually, GSVA transforms a p-gene by n-sample gene expression matrix into a g-geneset by n-sample pathway enrichment matrix.",
          "This facilitates many forms of statistical analysis in the 'space' of pathways rather than genes, providing a higher level of interpretability.",
      ...
      if (input$trim_labels == TRUE) {
        labels_row <- substr(
          x = gsub(
            ...
            replacement = " ",
            x = gsub(
              pattern = "^KEGG_|^REACTOME_|^BIOCARTA_|^GO_|^HALLMARK|",
              replacement = "",
              x = rownames(heatmap_data)
            )
          ),
          start = 1,
          stop = 40
